...
from core.repository.orders import get_last_buy_price
...
class TradingBot:
    def __init__(self, client):
        ...
        self.reentry = {"waiting": False, "prices": deque(maxlen=1000)}

        self.market = {"volatility": None, "market_type": None, "force": None}
        ...
        if self.wallet["btc"] == 0:
            if self.reentry["waiting"]:
                return evaluate_reentry(
                    price, self.market["market_type"], self.reentry["prices"]
                )
    ...
    def log_market(self, price):
        ...
        if (
            current_timestamp - self.last_market_log >= 1
